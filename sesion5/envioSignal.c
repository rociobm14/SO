...
int main(int argc, char *argv[]){
    ...
    int pid; //he tenido que cambiar long int por int porque sino me daba error
    int signal;
    if(argc<3) {
    ...
    pid= strtol(argv[2],NULL,10); //se usa para pasarlo a long int?
    ...
    signal=atoi(argv[1]); //no se puede hacer directamente signal = argv[1]?
    switch(signal) {
        
        case 0: //SIGTERM
            kill(pid,SIGTERM); //kill(pid_t, signal), pid_t se puede decir que es un int type.
            break;
